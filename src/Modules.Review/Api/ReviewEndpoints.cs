...
public static class ReviewEndpoints
{
    ...
    private static IResult ConfigurePolicy(
        ...
        IAuditWriter audit)
    {
        if (!auth.Has(context, "review.manage")) return PermissionAuthorizer.Forbidden();
        try
        {
    ...
    {
        var change = service.Find(id); if (change is null) return Results.NotFound();
    ...
    {
        if (!auth.Has(context, "review.merge")) return PermissionAuthorizer.Forbidden();
        try
        {
    ...
    {
        if (!auth.Has(context, permission)) return PermissionAuthorizer.Forbidden();
        try
        {
