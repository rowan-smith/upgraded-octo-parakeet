...
public sealed class InMemoryGitRepositoryStore : IGitRepositoryStore
{
    ...
    public IReadOnlyList<GitRepository> List() { lock (_gate) return _repositories.ToArray(); }
    public GitRepository? Find(Guid id) { lock (_gate) return _repositories.FirstOrDefault(repository => repository.Id == id); }
    public GitRepository? Find(string slug) { lock (_gate) return _repositories.FirstOrDefault(repository => repository.Slug.Equals(slug, StringComparison.OrdinalIgnoreCase)); }
    public void Add(GitRepository repository) { lock (_gate) _repositories.Add(repository); }
