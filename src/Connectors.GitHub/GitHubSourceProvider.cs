...
public sealed class GitHubSourceProvider(GitHubApiClient api) : ISourceProvider, IChangeSourceProvider
{
    ...
    private static RepositoryId ParseRepository(string value)
    {
        ...
        if (!normalized.StartsWith("http", StringComparison.OrdinalIgnoreCase)) normalized = $"https://{normalized}";
        ...
        if (parts.Length != 2) throw new ArgumentException("The GitHub URL must identify one repository.");
