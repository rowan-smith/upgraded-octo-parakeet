...
public sealed class SourceConnectionSeeder(IConfiguration configuration, ISourceConnectionStore connections, PlatformContextStore context)
{
    public void Seed()
    {
        ...
        if (string.IsNullOrWhiteSpace(provider) || string.IsNullOrWhiteSpace(owner) || string.IsNullOrWhiteSpace(name)) return;
        var repositoryId = new RepositoryId(provider, owner, name);
        if (connections.Find(context.Project.Id, repositoryId) is not null) return;
