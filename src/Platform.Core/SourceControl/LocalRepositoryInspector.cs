...
public sealed class LocalRepositoryInspector
{
    ...
    {
        if (string.IsNullOrWhiteSpace(path)) throw new ArgumentException("A local repository path is required.");
        var fullPath = Path.GetFullPath(path.Trim().Trim('"'));
        if (!Directory.Exists(fullPath)) throw new DirectoryNotFoundException($"Path '{fullPath}' does not exist.");
