...
public sealed class SeededNavigationPlaywrightTests(PlaywrightBrowserFixture browser)
{
    ...
    public async Task Overview_shows_organisation_and_project_context()
    {
        ...
        await Assertions.Expect(page.Locator("#content h1").First).ToContainTextAsync("ForgeDeck");
        await Assertions.Expect(page.Locator("#content")).ToContainTextAsync("ForgeDeck");
    }
}
