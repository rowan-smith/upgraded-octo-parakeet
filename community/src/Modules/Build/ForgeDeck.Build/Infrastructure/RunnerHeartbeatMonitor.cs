    ...
    ILogger<RunnerHeartbeatMonitor> logger) : BackgroundService
{
    ...
    private void Sweep()
    {
        ...
        foreach (var runner in store.ListRunners())
        {
            ...
            if (runner.LastHeartbeatAt is null)
            {
                continue;
            }

            if (now - runner.LastHeartbeatAt.Value < offlineAfter)
            {
                continue;
            }
        ...
        foreach (var run in store.ListRuns(200).Where(r => r.Status == PipelineRunStatus.Running))
        {
            ...
            {
                if (job.RunnerId is not Guid runnerId)
                {
                    continue;
                }

                var runner = store.FindRunner(runnerId);
                if (runner is null || runner.Status != RunnerStatus.Offline)
                {
                    continue;
                }
                ...
                if (now - runner.LastHeartbeatAt.Value < lostAfter)
                {
                    continue;
                }
