...
public sealed class NativeGitSourceProvider(IGitRepositoryStore repositories) : ISourceProvider, IChangeSourceProvider
{
    ...
    private static ExternalChange BuildChange(ForgeDeck.Git.Domain.GitRepository repository, string externalId, string? title = null, string? description = null, string? source = null, string? target = null)
    {
    ...
    private static SourceRepository ToSource(ForgeDeck.Git.Domain.GitRepository repository) =>
        new("native-git", "northstar", repository.Slug, repository.DefaultBranch, $"native://northstar/{repository.Slug}");

    private ForgeDeck.Git.Domain.GitRepository Require(string slug) =>
    ...
    private ForgeDeck.Git.Domain.GitRepository RequireByUrl(string repositoryUrl)
    {
