    ...
    IServiceProvider? services = null)
{
    ...
    public void SelectCommunityLicence()
    {
        ...
        if (licences is null)
        {
            throw new InvalidOperationException("Licensing is unavailable.");
        }
    ...
    public void AcknowledgeModules()
    {
        ...
        if (instance.LicenceMode == LicenceMode.None)
        {
            throw new InvalidOperationException("Select a licence before choosing modules.");
        }
...
public sealed class MembershipService(ITenancyStore store, IPasswordHasher<UserAccount> passwords)
{
    ...
    {
        if (!SlugRules.IsValidUsername(username))
        {
            throw new ArgumentException("Username is invalid.");
        }
...
public sealed class ProjectService(ITenancyStore store, ExtensionLifecycleService? extensions = null)
{
    ...
    public void GrantUser(Guid projectId, Guid userId, Guid? roleId = null)
    {
        ...
        if (store.FindUser(userId) is null)
        {
    ...
    public void GrantTeam(Guid projectId, Guid teamId, Guid? roleId = null)
    {
        ...
        if (store.FindTeam(teamId) is null)
        {
...
public sealed class ProjectAccessService(ITenancyStore store)
{
    ...
    {
        if (role is OrganisationRole.Owner or OrganisationRole.Admin)
        {
            return true;
        }

        if (project.Visibility == ProjectVisibility.Organisation)
        {
            return true;
        }

        if (store.HasProjectUserAccess(project.Id, userId))
        {
            return true;
        }
...
public sealed class TeamService(ITenancyStore store)
{
    public Team Create(string name, string? slug, string? description, Guid? roleId = null)
    {
        ...
        {
            throw new ArgumentException("Team slug is invalid.");
        }

        if (store.FindTeamBySlug(normalized) is not null)
        {
...
public sealed class RepositoryService(ITenancyStore store, ISourceConnectionStore sourceConnections)
{
    public Repository CreateRepository(Guid projectId, CreateRepositoryRequest request)
    {
        ...
        if (!SlugRules.IsValid(slug))
        {
            throw new ArgumentException("Repository slug is invalid.");
        }
